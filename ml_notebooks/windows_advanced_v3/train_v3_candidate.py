...
def label_process(state: ProcessContextState) -> str:
    ...
    # Label net.exe / net1.exe user/group modifications as Attack
    if image_name in ("net.exe", "net1.exe"):
        sensitive_args = ("/add", "/delete", "localgroup", "user", "/user", "/domain")
...
def load_data():
    ...
    files = [f for f in os.listdir(_TEMP_DIR) if f.endswith(".json")]
    
    for filename in files:
        filepath = _TEMP_DIR / filename
        
        # Categorize day/file group
        if "day1" in filename:
            group_label = "day1"
        elif "day2" in filename:
            group_label = "day2"
        elif "seatbelt" in filename:
            group_label = "seatbelt"
        elif "sharpview" in filename:
            group_label = "sharpview"
        elif "httplistener" in filename:
            group_label = "psh_http"
        elif "webserver" in filename:
            group_label = "psh_py"
        else:
            group_label = "custom"
            
        print(f"Ingesting {filename} as group {group_label}...")
    ...
    for guid, state in aggregator.processes.items():
        ...
        # USE THE CANDIDATE V3 EXTRACTOR HERE
...
def main():
    ...
    # Base XGBoost estimator configuration
    ...
    if xgb:
        ...
        evaluate_thresholds(X, y, groups, xgb_clf, "XGBoost V3 Candidate")
        
    rf_clf = RandomForestClassifier(n_estimators=100, max_depth=4, class_weight="balanced", random_state=42)
    evaluate_thresholds(X, y, groups, rf_clf, "Random Forest V3 Candidate")
    
    if xgb_clf is None:
        print("XGBoost not available. Exiting.")
        return
        
    print("\nTraining final Candidate XGBoost model on full dataset...")
    xgb_clf.fit(X, y)
    
    # Save the V3 candidate model and metadata payload
    model_path = _MODEL_DIR / "windows_advanced_v3_candidate.pkl"
    print(f"Saving final candidate model payload to {model_path}...")
    
    payload = {
        "model": xgb_clf,
        "model_version": "windows_advanced_v3_candidate",
        "feature_version": "v3_candidate",
        "feature_names": WindowsAdvancedV3CandidateFeatureExtractor.FEATURE_NAMES,
        "feature_count": len(WindowsAdvancedV3CandidateFeatureExtractor.FEATURE_NAMES),
        "class_mapping": {0: "Normal", 1: "Attack"},
        "threshold": 0.90,
        "probability_type": "raw_xgboost_probability",
        "training_dataset_identifier": "OTRF Expanded Day1/2/Seatbelt/Sharpview/Http/Py",
        "evaluation_methodology": "GroupKFold by filename group",
        "random_seed": 42,
    ...
    print("V3 Candidate Model Serialization completed successfully!")
